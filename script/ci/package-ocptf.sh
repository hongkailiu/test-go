...
readonly BUILD_FILE="${APP_FOLDER}/build/ocptf"
readonly PKG_DIR_NAME="pkg-ocptf"
...
readonly PKG_BASENAME="ocptf-${VERSION}-${GO_OS}-${GO_ARCH}.tar.gz"
...
current_dir="$(pwd)"
cd "${BUILD_DIR}" || exit 1
tar -czf "${PKG_FULLNAME}" --transform "s/${PKG_DIR_NAME}/ocptf/" "${PKG_DIR_NAME}"
