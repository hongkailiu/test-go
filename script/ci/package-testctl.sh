...
readonly BUILD_FILE="${APP_FOLDER}/build/testctl"
readonly PKG_DIR_NAME="pkg-testctl"
...
readonly PKG_BASENAME="testctl-${VERSION}-${GO_OS}-${GO_ARCH}.tar.gz"
...
current_dir="$(pwd)"
cd "${BUILD_DIR}" || exit 1
tar -czf "${PKG_FULLNAME}" --transform "s/${PKG_DIR_NAME}/testctl/" "${PKG_DIR_NAME}"
