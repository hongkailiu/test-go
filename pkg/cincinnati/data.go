...
// TODO: use graph-data while forming the graph, instead of adding first and removing later
...
func (gd CincinnatiGraphData) BecomeConditional(from Node, to string) ConditionalEdge {
	...
	for i, blockedEdge := range gd.BlockedEdges {
		...
		if gd.BlockedEdges[i].FromRegex.MatchString(fmt.Sprintf("%s+%s", from.Version.String(), from.Arch)) {
